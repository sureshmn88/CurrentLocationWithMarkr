...
public class GeocodeAddressIntentService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (addresses == null || addresses.size()  == 0) {
            ...
            deliverResultToReceiver(Constants.FAILURE_RESULT, errorMessage, null,errMsg);
        } else {
                ...
                try {
                    if (address.getAddressLine(0) != null) {
                        if(!address.getAddressLine(0).isEmpty())
                            ...
                            addressFragments.add("Address not return");
                    } else {
                        addressFragments.add("No address found");
                    }
                } catch (Exception ignored) {
                    errMsg=ignored.toString();
                }
            }
            ...
            deliverResultToReceiver(Constants.SUCCESS_RESULT,
                    TextUtils.join(System.getProperty("line.separator"),
                            addressFragments), address,errMsg);
        }
    }

    private void deliverResultToReceiver(int resultCode, String message, Address address,String errorMsg) {
        Bundle bundle = new Bundle();
        bundle.putParcelable(Constants.RESULT_ADDRESS, address);
        bundle.putString(Constants.RESULT_DATA_KEY, message);
        bundle.putString(Constants.RESULT_ERR_MSG, errorMsg);
